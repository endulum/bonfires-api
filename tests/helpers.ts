...
*/
export async function token(username: string): Promise<string> {
  const user: UserDocument = await User.findOne().byNameOrId(username);

  if (!user) {
    console.error(
      `Given user ${username} does not exist. Users that exist: `,
      (await User.find({})).map((u) => u.username)
    );
